...
func TestTryResumeStreamRecord_ResumeInMultipleBlocks_BalanceNotEnoughFinally(t *testing.T) {
	...
	// there will be a auto settle record
...
func TestTryResumeStreamRecord_ResumeInMultipleBlocks_DuringAutoSettle(t *testing.T) {
	keeper, ctx, depKeepers := makePaymentKeeper(t)
	ctx = ctx.WithBlockTime(time.Now())
	...
	// resume stream record which is during auto settle
	err := keeper.TryResumeStreamRecord(ctx, userStreamRecord, userStreamRecord.NetflowRate.Neg().SubRaw(10).MulRaw(int64(params.VersionedParams.ReserveTime)))
	...
	// further add balance
	err = keeper.TryResumeStreamRecord(ctx, userStreamRecord, userStreamRecord.FrozenNetflowRate.Neg().MulRaw(int64(params.VersionedParams.ReserveTime)))
	require.NoError(t, err) //only added static balance
	found = keeper.ExistsAutoResumeRecord(ctx, ctx.BlockTime().Unix(), user)
	require.True(t, found)
	userStreamRecord, _ = keeper.GetStreamRecord(ctx, user)
